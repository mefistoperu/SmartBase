...
function agregar(id,nombre,precio,afectacion,por1,por2,precio1,precio2)
{
	...
	var fila='<tr id="fila'+cont+'">'+
	         ...
	         '<td>'+cont+'</td>'+
	          ...
	          '<td><span id="subtotal'+cont+'" name="subtotal">'+subtotal+'</span><input type="hidden" id="afectacion'+cont+'" name="afectacion[]" class="form-control" value="'+afectacion+'"></td>'+
	         '</tr>';
...
function modificarPrecioVenta()
{
	...
	for(var i=0; i< cos.length;i++)
	{
		...
		inpPv.value = (parseFloat(inpC.value)+parseFloat(inpC.value*inpP1.value/100)).toFixed(2);
		document.getElementsByName("precio1"+[i]).innerHTML=inpPv.value;

		inpPvm.value = (parseFloat(inpC.value)+parseFloat(inpC.value*inpP2.value/100)).toFixed(2);
