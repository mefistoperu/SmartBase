...
if($_POST['action'] == 'ediCliente')
{
	...
	$query=$connect->prepare("UPDATE tbl_contribuyente SET correo=?,direccion=?,departamento=?,provincia=?,distrito=? WHERE id_persona = ?");
	$resultado = $query->execute([$correo,$direccion,$departamento,$provincia,$distrito,$id]);
